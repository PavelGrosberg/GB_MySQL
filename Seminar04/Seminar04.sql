DROP DATABASE IF EXISTS seminar4_db;
CREATE DATABASE seminar4_db;
USE seminar4_db;
...
SELECT `c`.`name`, `s`.`id` FROM `cats` `c` JOIN `shops` `s` ON `c`.`shops_id` = `s`.`id`;
SELECT `c`.`name`, `s`.`shopname` FROM `shops` `s` JOIN `cats` `c` ON `c`.`shops_id` = `s`.`id` WHERE `c`.`name` = 'Murzik';
SELECT `s`.* FROM `shops` `s` WHERE `s`.`id` NOT IN(
    SELECT `s1`.`id` FROM `shops` `s1` JOIN `cats` `c` ON `s1`.`id` = `c`.`shops_id`
    WHERE `c`.`name` IN ('Murzik', 'Zuza')
);
SELECT `a`.`an_name`, `a`.`an_price`, `o`.`ord_datetime` FROM `analysis` `a` JOIN `Orders` `o` ON `a`.`an_id` = `o`.`ord_an`
WHERE DATE(`o`.`ord_datetime`) BETWEEN '2020-02-05' AND DATE_ADD('2020-02-05', INTERVAL 7 DAY) ORDER BY `o`.`ord_datetime`;
